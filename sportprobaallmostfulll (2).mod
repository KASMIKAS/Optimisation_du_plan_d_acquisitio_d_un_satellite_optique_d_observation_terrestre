...
maximize sum(ima in Images) 
	(PA[ima]*selection[ima]*probaInstrumentOK[ima]);
...
constraints {
   ...
   // contrainte de la memoire
   sum(im in Images) PM[im] * selection[im] <= PMmax;
   
   // assignation des images sélectionnées
	forall(ima in Images,ins in Instruments) {
		assignedTo[ima, ins] <= selection[ima];
	}
	
	// calcul de des probabilités
	forall(ima in Images) {
    	probaInstrumentOK[ima] == 
        	(TY[ima] == 2 ? 
            	(1 - Failure[1]) * (1 - Failure[3]) * (1 - ProbaSup[ima]) 
        	: 
            	sum(ins in Instruments) (assignedTo[ima, ins] * (1 - Failure[ins]) * (1 - ProbaSup[ima])));
	}   
}
